...
export const ModalProvider = ({ children }) => {
  ...
  return (
    <ModalContext.Provider
      value={{ modalOpen, modalType, modalProps, openModal, hideModal }}
    >
      {children}
      {modalOpen &&
        ReactDOM.createPortal(
          ...
          >
            ...
            <div
              className={`relative p-6 rounded shadow-lg z-10 w-full max-w-md container-style border-none transition-all duration-300 ease-out transform ${
            ...
            >
              ...
              {/* Title (if provided) */}
              {modalProps.title && (
                <h2 className="text-2xl font-bold mb-4">{modalProps.title}</h2>
              )}
              {/* Description (if provided) */}
              {modalProps.description && (
                <p className="mb-4">{modalProps.description}</p>
              )}
                ...
                {modalProps.confirmButtonText && (
                  <button
                    className={`px-4 py-2 rounded text-white ${
                      modalProps.confirmButtonType === 'Danger'
