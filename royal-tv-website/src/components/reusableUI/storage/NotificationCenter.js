 ...
 * 🔔
 * NOTIFICATION CENTER – Simple preview, paginated drawer, smooth expand!
 * - Preview top 3, then paginated drawer (7 first, then 10 per "next").
 * - No pagination component, all buttons in bottom bar, smooth appearance.
 * - No auto-scrolling/jumping! User stays in place.
 * ===================================================================
 */
...
import RefreshNotificationsButton from '@/components/reusableUI/socket/RefreshNotificationsButton';
...
export default function NotificationCenter({ userRole = 'user' }) {
  ...
  const fetchNotifications = useCallback(async () => {
    ...
    try {
      ...
      setNotifications(sortNotificationsByUnreadAndDate(res.data.notifications || []));
  ...
  const toggleExpanded = (id, notif) => {
    // If opening an unread, optimistically mark as read locally
    if (!notif.is_read && !expandedIds[id]) {
      // Update local state: set is_read true for this notification
      setNotifications((prevNotifs) =>
        prevNotifs.map((n) => (n.notification_id === id ? { ...n, is_read: true } : n))
      );
      // Send to backend (async, no state update)
      markAsRead(id); // markAsRead should NOT refetch notifications
      setExpandedIds((prev) => ({
        ...prev,
        [id]: true
      }));
    ...
    // If closing, re-fetch & sort to push this notification down
    if (notif.is_read && expandedIds[id]) {
      setExpandedIds((prev) => ({
        ...prev,
        [id]: false
      }));
      fetchNotifications(); // Will sort and update!
      return;
    }
  ...
  // 🔄 Helper: always sort notifications, unread first, newest first
  ...
  // Drawer slice logic (always after the preview, up to the current "page")
  let drawerSlice = [];
  if (drawerOpen) {
    // Gather all drawer pages so far (for "expand" effect)
            ...
            <div key={notif.notification_id} className={notificationCardClasses(notif)}>
              ...
              >
                ...
                >
                  ...
                  <div className="mt-2 pt-3 border-t border-gray-800 text-md">
                    <div>{new Date(notif.createdAt).toLocaleString()}</div>
                    {userRole === 'admin' && notif.type && <div>Type: {notif.type}</div>}
                  </div>
                </div>
              </div>
            </div>
            ...
            {moreToShow && (
              <button
                ...
                onClick={() => {
                  ...
                  // No scrollToNew, we just expand the batch!
                }}
              >
                👇 View next 10
              </button>
