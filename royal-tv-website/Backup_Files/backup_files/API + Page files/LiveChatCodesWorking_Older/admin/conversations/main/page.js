...
const ConversationsPage = () => {
  ...
  const fetchUsersWithConversations = async () => {
    try {
      ...
      const response = await axiosInstance.get('/api/admin/conversations/main', {
  ...
  const handleDeleteConversations = async (user) => {
    openModal('deleteAllConversations', {
      ...
      confirmButtonClassName: 'bg-red-600 hover:bg-red-700 text-white font-bold px-4 py-2 rounded',

      cancelButtonText: 'Cancel',
      onConfirm: async () => {
        try {
          showLoader({ text: 'Deleting all conversations from user...' });
          await axiosInstance.delete('/api/admin/conversations/main/deleteAllUserConversations', {
                ...
                <tr key={user.user_id} className="hover:bg-gray-400">
                  ...
                  <td className="border border-gray-300 px-4 py-2">
                    <div className="flex flex-row gap-2 items-center justify-center no-wrap">
                      <Link href={`/admin/conversations/user/${user.user_id}`}>
