/**
 *   ================== userNotificationsTest.js ==================
 * 🧑‍💻
 * DEV ROUTE – TEST USER NOTIFICATIONS
 * - Triggers every user notification type with complete dummy data.
 * - Pass {type} in POST body: 'freeTrialRequested', 'freeTrialActivated', etc.
 * =====================================================================
 */

import { userNotificationBuilder } from '@/constants/notificationSystem';
...
export async function POST(request) {
  ...
  switch (type) {
    case 'newUserRegistration':
      notification = userNotificationBuilder.newUserRegistration(dummyUser);
      break;
    case 'freeTrialRequested':
      notification = userNotificationBuilder.freeTrialRequested(dummyFreeTrial, dummyUser);
      break;
    case 'freeTrialActivated':
      notification = userNotificationBuilder.freeTrialActivated(dummyFreeTrial, dummyUser);
      break;
    case 'subscriptionCreated':
      notification = userNotificationBuilder.subscriptionCreated(dummySubscription, dummyUser);
      break;
    case 'subscriptionActivated':
      notification = userNotificationBuilder.subscriptionActivated(dummySubscription, dummyUser);
      break;
    case 'paymentReceived':
      notification = userNotificationBuilder.paymentReceived(dummyPayment, dummyUser);
      break;
    case 'liveChatMessage':
      notification = userNotificationBuilder.liveChatMessage(
        dummyChatMessage,
        dummyConversation,
        dummyUser
      );
    ...
    case 'bubbleChatMessage':
      notification = userNotificationBuilder.bubbleChatMessage(
        dummyChatMessage,
        dummyConversation,
        dummyUser
      );
