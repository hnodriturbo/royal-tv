...
const AdminLiveChatConversation = () => {
  ...
  const fetchConversationAndList = async () => {
    try {
      ...
      const { data } = await axiosInstance.get(`/api/admin/liveChat/${conversation_id}`);
      ...
      const list = await axiosInstance.get(`/api/admin/liveChat/user/${data.user.user_id}`);
  ...
  useEffect(() => {
    ...
    /* receive_message */
    const unSubReceive = listen('receive_message', (msg) => {
      if (msg.conversation_id !== conversation_id) return;
      setMessages((m) => [...m, msg]);
      setSeenIds((ids) => ids.filter((id) => id !== msg.message_id));
    });

    /* messages_marked_read */
    const unSubRead = listen('messages_marked_read', ({ conversation_id: id }) => {
      if (id !== conversation_id) return;
      setMessages((m) =>
        m.map((item) => (item.status === 'sent' ? { ...item, status: 'read' } : item))
      );
    });

    /* message_edited */
    const unSubEdit = listen('message_edited', (edited) =>
      setMessages((curr) =>
        curr.map((m) =>
          m.message_id === edited.message_id
            ? { ...m, message: edited.message, updatedAt: edited.updatedAt }
            : m
        )
      )
    );
    ...
    /* message_deleted */
    const unSubDelete = listen('message_deleted', ({ message_id }) =>
      setMessages((curr) =>
        curr.map((m) => (m.message_id === message_id ? { ...m, status: 'deleted' } : m))
      )
    );
    ...
    openModal('editMessage', {
      ...
      onConfirm: async () => {
        ...
        try {
          ...
          await axiosInstance.put(`/api/admin/conversations/${conversation_id}`, {
    ...
    openModal('confirmDelete', {
      ...
      onConfirm: async () => {
        try {
          ...
          await axiosInstance.delete(`/api/admin/conversations/${conversation_id}`);
    ...
    <div className="flex flex-col items-center justify-center w-full">
      <div className="container-style">
        ...
        <div className="flex flex-col items-center justify-center">
          <div className="container-style w-10/12 items-center my-5 lg:min-h-fit">
            ...
            {Object.values(othersTyping).some(Boolean) && (
              <p className="text-center italic text-sm text-gray-300 mb-1">Someone is typing…</p>
            )}
